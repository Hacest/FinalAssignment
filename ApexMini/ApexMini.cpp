	...
	skill->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)-> bool
		{
			...
			if (turn == 0 && CausticSelect == 1 && CausticTurn > 0)
			{
				...
				Caustic->setScale(0.5);
				gasTimer = 1;
				if (gasX >= LifelineX - 150 && gasX <= LifelineX && gasY >= LifelineY - 100 && gasY <= LifelineY + 50)
					Lifeline->hide();
				if (gasX >= WraithX - 150 && gasX <= WraithX && gasY >= WraithY - 100 && gasY <= WraithY + 50)
					Wraith->hide();
				if (gasX >= GibraltarX - 150 && gasX <= GibraltarX && gasY >= GibraltarY - 100 && gasY <= GibraltarY + 50)
					Gibraltar->hide();
				if (gasX >= MirageX - 150 && gasX <= MirageX && gasY >= MirageY - 100 && gasY <= MirageY + 50)
					Mirage->hide();
				if (gasX >= Bot1X - 150 && gasX <= Bot1X && gasY >= Bot1Y - 100 && gasY <= Bot1Y + 50)
				{
					Bot1->hide();
					Bot1Life = 0;
				}
				if (gasX >= Bot2X - 150 && gasX <= Bot2X && gasY >= Bot2Y - 100 && gasY <= Bot2Y + 50)
				{
					Bot2->hide();
					Bot2Life = 0;
				}
				if (gasX >= Bot3X - 150 && gasX <= Bot3X && gasY >= Bot3Y - 100 && gasY <= Bot3Y + 50)
				{
					Bot2->hide();
					Bot2Life = 0;
				}
